...
function MaterialDocumentView({ document, compact = false }: { document: NormalizedDocument; compact?: boolean }) {
  ...
  return <div className="normalized-document">
    <div className="material-meta"><span>{NORMALIZED_CATEGORY_LABELS[document.category]}</span><span>{document.pages} 页</span><span>{document.status === "complete" ? "提取完整" : "部分提取"}</span><span>校验 {document.checksum.slice(0, 8)}</span></div>
    {document.status !== "complete" && <p className="material-warning">这份资料存在缺页或 OCR 识别不完整，学习时请把异常文本视为待核对内容。</p>}
...
function MaterialsLibrary() {
  ...
  return <div className="page-content">
    <section className="hero-card material-hero"><div className="hero-copy"><span className="eyebrow">NORMALIZED ENGLISH II LIBRARY</span><h1>英语（二）学习资料</h1><p>这里完整收录 normalized 文件夹的 {normalizedCourse.documentCount} 份资料，共 {normalizedCourse.totalCharacters.toLocaleString()} 个正文字符。它们已经替换原来的通用演示课程，并进入每日路线。</p></div><div className="plan-card"><span>导入审计</span><div className="plan-step"><b>70</b><div><strong>全部文件已登记</strong><small>缺失 {normalizedCourse.audit.missingDocuments.length} 份</small></div></div><div className="plan-step"><b>{normalizedCourse.audit.vocabularyCount}</b><div><strong>词汇候选</strong><small>每天新增最多 30 个</small></div></div><div className="plan-step"><b>{normalizedCourse.audit.phraseCount}</b><div><strong>词组候选</strong><small>独立于每日单词上限</small></div></div></div></section>
...
function StudySessionView({ state, updateState, exit }: { state: AppState; updateState: (value: AppState) => void; exit: () => void }) {
  ...
  return <WordLookupProvider state={state}><div className="session-shell"><header className="session-header"><button className="icon-button" onClick={exit} aria-label="退出学习">×</button><div><span>{lesson.level} · {mode} 分钟</span><strong><InteractiveText text={lesson.title}/></strong></div><span>{stepIndex + 1} / {steps.length}</span></header><div className="session-progress">{steps.map((item, index) => <span key={item.id} className={index < stepIndex ? "done" : index === stepIndex ? "active" : ""}><i/>{item.label}</span>)}</div>
    <main className="session-main">
      ...
      {(step === "micro-read" || step === "read") && <section className="step-card reading-step"><span className="eyebrow">NORMALIZED MATERIAL · {lesson.sourceCategory}</span><h1><InteractiveText text={lesson.sourceTitle}/></h1><div className="reading-tools"><button className="secondary small" onClick={speak}>播放提取的英文句</button><button className="secondary small" onClick={() => setShowChinese(!showChinese)}>{showChinese ? "隐藏学习提示" : "显示学习提示"}</button><span>正文中的英文单词均可点击查词。</span></div>{sourceDocument ? <MaterialDocumentView document={sourceDocument} compact/> : <div className="article-body">{lesson.paragraphs.map((paragraph, index) => <div key={paragraph.en}><p><span className="paragraph-index">{index + 1}</span><InteractiveText text={paragraph.en} context={paragraph.en}/></p>{showChinese && <p className="translation">{paragraph.zh}</p>}</div>)}</div>}{showChinese && <p className="material-warning">先完成当前分页，再在“英语二资料”中查看整份文件。遇到 OCR 错字请结合上下文和词典核对。</p>}</section>}
