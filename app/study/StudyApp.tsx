...
function Onboarding({ state, onComplete }: { state: AppState; onComplete: (value: AppState) => void }) {
  ...
  return <div className="result-shell">
    <section className="result-card">
      ...
      <div className="route-preview"><span>74 天路线</span><strong>OCR 教材内容 → 核心词 → 单元复盘 → 考前回收</strong><small>每天仍可切换 45 / 90 / 150 分钟</small></div>
...
function Sidebar({ tab, setTab, state }: { tab: Tab; setTab: (tab: Tab) => void; state: AppState }) {
  ...
  return <aside className="sidebar">
    ...
    <div className="sidebar-status"><span>74 天路线</span><strong>第 {week} 周 · {STAGE_TITLES[currentStage]}</strong><ProgressBar value={(week / 11) * 100} /><small>{state.sessions.length} 次真实学习记录</small></div>
  </aside>;
}

function MaterialDocumentView({ document, compact = false, sectionIds = [] }: { document: NormalizedDocument; compact?: boolean; sectionIds?: string[] }) {
  const scopedSections = sectionIds.length ? document.sections.filter((section) => sectionIds.includes(section.id)) : document.sections;
  const visibleSections = compact ? scopedSections.slice(0, 4) : scopedSections;
  return <div className="normalized-document">
    ...
    {compact && scopedSections.length > visibleSections.length && <p className="muted">本课先显示前 {visibleSections.length} 个单元；完整 {scopedSections.length} 个单元可在“英语二资料”中查看。</p>}
  </div>;
}
...
function MaterialsLibrary() {
  ...
  return <div className="page-content">
    <section className="hero-card material-hero"><div className="hero-copy"><span className="eyebrow">TEXTBOOK · ENGLISH II</span><h1>英语（二）学习资料</h1><p>这里完整收录 《英语（二）自学教程》（2012年版）的 {normalizedCourse.documentCount} 份 OCR 派生资料，共 {normalizedCourse.totalCharacters.toLocaleString()} 个正文字符。它们已经替换原来的通用演示课程，并进入每日路线。</p></div><div className="plan-card"><span>教材来源</span><div className="plan-step"><b>{normalizedCourse.documentCount}</b><div><strong>全部文件已登记</strong><small>缺失 {normalizedCourse.audit.missingDocuments.length} 份</small></div></div><div className="plan-step"><b>{normalizedCourse.audit.vocabularyCount}</b><div><strong>核心词汇</strong><small>每天新增最多 35 个</small></div></div><div className="plan-step"><b>{normalizedCourse.audit.phraseCount}</b><div><strong>教材短语</strong><small>进入查词与复盘</small></div></div></div></section>
...
function Route({ state, updateState }: { state: AppState; updateState: (value: AppState) => void }) {
  ...
  return <div className="page-content"><div className="page-heading"><span className="eyebrow">YOUR ROUTE</span><h1>阶段锁定的 74 天路线</h1><p>从完整 OCR 教材 JSON 提取对话、练习、词汇和短语，按 10.23 考试日倒排。</p></div>
    <div className="route-layout"><section className="route-list">{phaseDefinitions.map((phase, index) => { const current = phase.id === currentStage; const finished = phase.weeks[1] < week && phase.id !== currentStage; return <article className={current ? "route-stage current" : finished ? "route-stage finished" : "route-stage"} key={phase.id}><span className="route-index">{finished ? "✓" : index + 1}</span><div><small>第 {phase.weeks[0]}-{phase.weeks[1]} 周</small><h2>{phase.title}</h2><p>{phase.goal}</p><p>晋级条件：{Object.entries(phase.required).map(([ability, score]) => `${ABILITY_LABELS[ability as Ability]} ${score}`).join(" / ")}</p>{current && <span className="current-tag">当前阶段 · {promotion.passed ? "已达标" : "补救中"}</span>}</div></article>; })}</section>
      <aside className="panel sticky-card"><span className="eyebrow">当前阶段</span><h2>{STAGE_TITLES[currentStage]}</h2><p>{STAGE_GOALS[currentStage]}</p><hr/><strong>晋级差距</strong><p>{getPromotionGapText(state, currentStage)}</p><button className="secondary wide" onClick={() => updateState(recordWeeklyCheck(state))}>记录一次周检测/补救判断</button><hr/><strong>OCR 日程</strong><p>已生成 {lessons.length} 天教材计划，覆盖到 2026-10-23。</p><ProgressBar value={(new Set(state.sessions.map((item) => item.lessonId)).size / lessons.length) * 100} /></aside>
    </div></div>;
}
